...
function ServerChannel({ channel, server, role }: IServerChannel) {
  const Icon = iconMap[channel.type]
  const params = useParams()
  const router = useRouter()
  const { onOpen } = useModal()

  const HanldeActionClick = (e: React.MouseEvent, action: ModalType) => {
    e.preventDefault()
      ...
      {channel.name !== 'general' && role !== MemberRole.GUEST && (
        <div className="ml-auto flex items-center gap-x-2">
          <ActionTooltip label="Edit">
            <Edit
              onClick={(e) => HanldeActionClick(e, 'editChannel')}
              className="hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 transition"
            />
          </ActionTooltip>
          <ActionTooltip label="Delete">
            <Trash
              onClick={(e) => HanldeActionClick(e, 'deleteChannel')}
              className="hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 transition"
            />
          </ActionTooltip>
        </div>
