...
function MembersModal() {
  ...
  const onRoleChange = async (memberId: string, role: MemberRole) => {
    try {
      ...
      toast({
        title: 'Something went wrong',
        description: 'An error occured at role change',
      })
    } finally {
      setLoadingId('')
    }
    ...
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="bg-white text-black p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          <DialogTitle className="text-2xl text-center">Manage Members</DialogTitle>
        ...
        <div className="p-6">
          <ScrollArea>
            {server?.members?.map((member) => (
              <div key={member.id} className="flex itmsce gap-x-2 mb-6">
                ...
                {server.profileId !== member.profileId && loadingId !== member.id && (
                  <div className="ml-auto">
                    <DropdownMenu>
                      <DropdownMenuTrigger>
                        <MoreVertical className="h-5 w-5 text-zinc-500 hover:text-zinc-600" />
                      </DropdownMenuTrigger>
